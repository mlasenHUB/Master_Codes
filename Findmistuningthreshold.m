%% FIND THRESHOLD: finds the threshold at which the mistuning cause the optimum force with Tpe B/approach 3 to change in a tuned and mistuned case

%trying 10 cases of mistuning from 1 to 10%
mistcases=10;
fsTT=zeros(12,mistcases); %opt forces for 10 mist cases
fsMT=zeros(12, mistcases);
modes=12; %modes to study
...
for s=1:modes
    ...
    file=(['k2s', num2str(s)])';%2:blades only(m-2-s, k-2-s) 1:all dofs
    %%%%%%%%%%%%%%%%
