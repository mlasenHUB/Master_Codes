...
%and 1 Piezo per mode suppression
...
ppm=1;% piezos per mode to suppressed (1=1 piezo per mode, 2=2 piezos per mode)
...
for s=1:modes
    ...
    file=(['OPTs', num2str(s),'-ppm=1real'])';%A=1=>real
    %%%%%%%%%%%%%%%%
...
%%
msfT=max(MSFfT); %all optimium msf per mode
msfM=max(MSFfM);
