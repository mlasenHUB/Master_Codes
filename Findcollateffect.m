%% Compare the effect of one mode optimal in the rest modes(collateral effect) and save the file
...
msfcol=zeros(modes,modes);
msfcurr=zeros(modes,modes);%msf from the optimum of the current mode
bodcol=zeros(Nb,modes, modes);%displacement of blades only (with sOr opt)
bodcurr=zeros(Nb,modes, modes);
for sOR=1:modes %original moal optimum
    for s=1:modes %collateral modes (original modes is sOR)
        sOR,s
        %%%%%%%%%%%%%%%%
        %s= Mode to be supressed with the window(up to Nb+N)
        fileor=(['OPTs', num2str(sOR),'boM'])';%run tuned and mistuned separately
        filecol=(['COLsOR',num2str(sOR),'ats',num2str(s), 'M']);
        %%%%%%%%%%%%%%%%
        ...
        if loadopt==1
           % [ DpT, sor , msfpT, msfcT, bodpT, bodcT ] = fixdpiezoloc( fileor, 0, VT, nrT, dT,s, f,DTT, OPTT, PPT, DOUBLE, ppm, 'TUNED' , bo, Nb);
            [ DpM, sor , msfpM, msfcM, bodpM, bodcM ] = fixdpiezoloc( fileor, 1, VM, nrM, dM,s, f,DMM, OPTM, PPM, DOUBLE, ppm, 'MISTUNED', bo, Nb );
        end
        
        %% Usefull vectors
        msfcol(sOR, s)=msfpM;%effect of modes sOR at mode s
        msfcurr(sOR,s)=msfcM;%msf from the optimum of the current mode
        bodcol(:,sOR,s)=bodpM(N+1:end);%displacement of blades only (with sOr opt)
        bodcurr(:,sOR,s)=bodcM(N+1:end);%displacement b-o with the current optimum.
